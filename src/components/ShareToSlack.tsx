...
function buildSlackMessage(d: DashboardData): string {
  ...
  // Sports sorted by turnover desc, only those with data
  const sports = [...d.sportsBreakdown]
    .filter(s => s.bets > 0 || s.turnover > 0)
    .sort((a, b) => b.turnover - a.turnover);

  // Users sorted by turnover desc
  const users = [...d.userSummaries].sort((a, b) => b.turnover - a.turnover);

  // Market patterns - group by sport (first word before colon or use market name)
  const marketLines = d.marketPatterns
    .filter(m => m.count > 0)
